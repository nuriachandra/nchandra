...
<div style="display: flex; align-items: center; gap: 20px;">
  ...
  <p style="margin: 0; font-style: italic; max-width: 300px;">LION-CLAP embeddings of audio clips of Donald Trump. Embeddings of AI-generated audio (orange) cluster separately from real audio samples.</p>
</div>
...
Preprint:
Nuria Alina Chandra, Yan Hu, Jason D. Buenrostro, Sara Mostafavi, Alexander Sasse. Refining sequence-to-activity models by increasing model resolution. [bioRxiv 2025.01.24.634804](https://www.biorxiv.org/content/10.1101/2025.01.24.634804v2). Jan. 24, 2025.
...
## Discrete Diffusion
*Research conducted with Andrew Gordon Wilson and Alan Amin at New York University.*

### Connecting discrete and continuous diffusion
To model discrete sequences such as DNA, proteins, and language using diffusion, practitioners must choose between three major methods: diffusion in discrete space, Gaussian diffusion in Euclidean space, or diffusion on the simplex. Despite their shared goal, these models have disparate algorithms, theoretical structures, and strengths: discrete diffusion is the most natural domain, Gaussian diffusion is more methodologically mature, and diffusion on the simplex bridges the strengths of the other two but suffers from numerically unstable stochastic processes. In this project we developed a theoretical framework to connect these three different domains, creating a system under which practitioners can seamlessly transition between the domains to best meet their needs. To do so, we apply mathematical tools from genetics. We show that discrete diffusion can be formulated using the Wright-Fisher model of population genetics. This then allows us to show that simplex diffusion and Gaussian diffusion are large-population limits of discrete diffusion! Our theory formally connects the likelihoods and hyperparameters of these models and leverages decades of mathematical genetics literature to unlock stable simplicial diffusion. Finally, we relieve the practitioner of balancing model trade-offs by demonstrating it is possible to train a single model that can perform diffusion in any of these three domains at test time. Our experiments show that Wright-Fisher simplicial diffusion is more stable and outperforms previous simplicial diffusion models on DNA and scales to language. We also show that we can train models on all three domains at once that are competitive with models trained on any individual domain. 

Ongoing work. Submitted to the Frontiers in Probabilistic Inference Workshop at NeurIPS 2025.
